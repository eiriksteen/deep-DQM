...
#     classifiers = nn.ModuleList([copy.deepcopy(classifier)
#                                 for _ in range(args.n_models)])

#     optimizers = [torch.optim.Adam(c.parameters(), lr=args.lr)
...
#     total_probs_per_model = torch.zeros(
#         args.n_models, len(data))
#     total_loss_per_step_per_model = torch.zeros(
#         args.n_models, len(data))
...
#             outs = [c(histogram, pastk[:len(histogram)]) for c in classifiers]
#             logits = [o["logits"] for o in outs]

#             loss = [loss_fn(l, is_anomaly) for l in logits]
#             total_loss_per_step_per_model[:, batch_num] = torch.stack(loss)

#         if batch_num == int(args.warmup_frac * len(data)):
#             print(f"WARMUP FINISHED. LOSS: {
#                   np.mean(total_loss_per_step_per_model[:, batch_num])}")

#         if batch_num > 0:

#             probs = torch.stack([torch.sigmoid(l) for l in logits])
